## Y8D00-CPU

### 介绍
  Minecraft实现基于[自创指令集架构](https://github.com/FISHduoduo123/Y8D00-CPU/blob/main/Instruction%20Set%20Architecture%201.0%20.pdf)设计CPU  
  该仓库提供 : 参考YDD_ISA_1.0.1指令集架构设计的CPU内核,及其基于该内核设计计算机系统等开发项目.此外提供了计算机编程编译烧写工具(所有硬件都在Java版Minecraft中实现,软件均为python程序,请确保准备py编程语言编译环境和IDE工具以及基于SpigotMC服务器).
### 规格：
  - 哈佛架构
  - 单周期CPU
  - 主频0.21Hz
  - 数据位宽8位
  - 指令位宽24位
  - 双总线接口
  - 最高可寻址256字节空间地址
  - 最高可挂载256个外围设备

### 使用说明
#### 1.放置计算机
  解压computer.zip,使用WorldDedit模组放置comuputer.schem蓝图<br>
  ```
        //schem load computer.schem
        //paste
  ```
此外提供了该单个处理器蓝图，后续将会补充该CPU具体硬件细节以便感兴趣的读者对其进行开发.
#### 2.查看计算机硬件
  计算机放置完成后找到图片中所标注位置:(图中"电"改为"点") <br>
  ![输入图片说明](image/49edbeec14ad2f392ca4743bdc22469d.png) <br>
  -顺序执行ROM烧录位置，如下图所示(FlashingTool/ctr_mc_1): <br>
  ![输入图片说明](image/5f67b13486a2ea80a670096f47ea3874.png) <br>
  -可寻址跳转ROM烧录位置，如下图所示(FlashingTool/ctr_mc_2): <br>
  ![输入图片说明](image/d4dbae94ec82546c7da9384ccd81cde2.png) <br>
   站立到蓝色方块上后，执行烧录程序，即可对其ROM的烧录. <br>
   烧录完毕后旁边可看到白色的房间嵌在计算机内，该房间负责对整个计算机的启动关闭以及运行模式进行控制. <br> 
  ![输入图片说明](image/10ce71cca7fb91e1f3b54e3493205d85.png) <br>
   下图为控制说明:<br>
  ![输入图片说明](image/7da627d1e4e40f2f87e74863500fb10c.png) 
  ![输入图片说明](image/d24da3ab7542c14921a49f63674d8dcd.png) <br>
#### 3. 启动计算机
    确保程序烧录好后，需要配置计算机启动模式：可寻址跳转ROM启动/顺序执行ROM启动，若为顺序执行ROM作为启动模式需将ROM进行一次复位. <br>
    配置好启动模式后来到如图中标注地方: <br>
![输入图片说明](image/17186ea2529ec3cf35c8719d21cf869f.png) <br>
    将PC寄存器中的每一位值手动置0. <br>
    回到控制房间找到时钟输入口，并接入如下图所示标准时钟源: <br>
![输入图片说明](image/c3e6cf9c771732c6bd107ceffea93997.png) <br>
    接入时钟源并确保时钟工作即可正常启动计算机,关闭时钟源即可使计算机停止工作,按照上述配置方式即可再次启动计算机. <br>
#### 汇编器使用说明
    汇编源代码在/ydd_isa_1.0.1_asm下,使用IDE打开该根目录并找到main.py设置汇编编译源文件和编译目标文件后即可运行程序进行编译. <br>
    如下图所示: <br>
![输入图片说明](image/acb47b693ab25077305d9d435e0b819f.png) <br>
    提供了三个汇编示例程序/bresenham(slope小于等于1）.txt /弹方块.txt /字符显示函数.txt 
    此外字符显示函数中的前三个指令立即数分别传入字符ASCII码、X偏移量、Y偏移量,如下图所示: 
![输入图片说明](image/29d9c14c217ef6b9284c263390a799e3.png) <br>
#### 烧录器使用说明
    该烧录器使用到了mcpy库，需要在SpigotMC服务器中实现烧录. 
    使用烧录其工具前将服务器插件文件中树莓派插件/raspberryjuice-1.12.jar 
    烧录程序中设置好烧录文件和玩家名称,如下图顺序执行ROM烧写程序为例: 
![输入图片说明](image/3e4fa03f20978c0cad0fab573fd2d3df.png) <br>
    设置完成站在要求指定的方块上运行程序即可完成机器码的烧录. <br>    

By:FISHduoduo
    
